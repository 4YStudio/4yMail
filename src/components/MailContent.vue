...
function sanitizeHtml(html) {
  ...
  // 1. 尝试提取 body 的样式以保留背景色等
  const bodyMatch = html.match(/<body\b([^>]*)>([\s\S]*?)<\/body>/i)
  let bodyAttrs = bodyMatch ? bodyMatch[1] : ''
  let content = bodyMatch ? bodyMatch[2] : html
  
  // 2. 基础清理，移除 script 标签和事件处理器
  ...
  // 4. 将提取的 body 属性（特别是 style）应用到一个包装层上
  // 这能帮助我们捕获邮件原本的背景颜色
  return `<div class="mail-body-wrapper" ${bodyAttrs}>${sanitized}</div>`
}
</script>
